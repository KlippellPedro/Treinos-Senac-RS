...
def create_guest(data):
    ...
    cursor = conn.cursor()
    try:
        query = """INSERT INTO convidado (id_evento, nome, sobrenome, cpf, telefone, email, numero_mesa, tipo_convidado) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"""
...
def checkin_guest(guest_id, user_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT id_checkin FROM checkin WHERE id_convidado=%s",(guest_id,))
        if cursor.fetchone():
            return False
