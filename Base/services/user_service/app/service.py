...
def list_users():
    ...
    cursor.execute("SELECT id_usuario, nome, cpf, email, id_perfil FROM usuario")
...
def get_user(user_id):
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id_usuario, nome, cpf, email, id_perfil FROM usuario WHERE id_usuario=%s", (user_id,))
...
def create_user(data):
    ...
    try:
        cursor.execute(
            "INSERT INTO usuario (nome, cpf, email, senha, id_perfil) VALUES (%s,%s,%s,%s,%s)",
            (data['nome'], data['cpf'], data['email'], hash_password(data['senha']), data['id_perfil'])
        )
        conn.commit()
...
def update_user(user_id, data):
    ...
    for key in ["nome", "email", "senha", "cpf", "id_perfil"]:
        if key in data:
            if key == "senha":
